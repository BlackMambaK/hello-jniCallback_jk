...
void queryRuntimeInfo(JNIEnv *env, jobject instance) {
    ...
    //在C/C++的程序，编译器提供了两个宏来支持取得源文件中的行号和文件名，这两个宏分别是__FILE__,__LINE__
    if (!versionFunc) {
...
void *UpdateTicks(void *context) {
    ...
    JNIEnv *env;
    jint res = (*javaVM)->GetEnv(javaVM, (void **) &env, JNI_VERSION_1_6);//获取当前线程的JNI接口指针 JNIEnv
    // 如果当前线程还没有attach到虚拟机，则设置 *env 为 NULL ，并返回 JNI_EDETACHED 。
    // 如果指定的JNI版本不被支持，则也设置 *env 为 NULL ，并且返回 JNI_EVERSION。否则设置 *env 为正常的接口，并返回 JNI_OK 。
    if (res != JNI_OK) {
        res = (*javaVM)->AttachCurrentThread(javaVM, &env, NULL);//attach当前线程到Java虚拟机，返回JNI接口指针 JNIEnv
        if (JNI_OK != res) {
...
Java_com_example_hellojnicallback_MainActivity_startTicks(JNIEnv *env, jobject instance) {
    pthread_t threadInfo_;//用于表示Thread ID,具体内容根据实现的不同而不同，有可能是一个Structure，因此不能将其看作为整数
    pthread_attr_t threadAttr_;

    pthread_attr_init(&threadAttr_);//对线程属性初始化
    pthread_attr_setdetachstate(&threadAttr_, PTHREAD_CREATE_DETACHED);

    pthread_mutex_init(&g_ctx.lock, NULL);//初始化锁变量mutex。attr为锁属性，NULL值为默认属性。
     ...
     * pthread_create
     * 参数
　　  第一个参数为返回最后创建出来的Thread的Thread ID.
　　  第二个参数用来设置线程属性.
　　  第三个参数是线程运行函数的起始地址.
　　  最后一个参数是运行函数的参数. 线程函数有多个参数的情况：这种情况就必须申明一个结构体来包含所有的参数，然后在传入线程函数
     * */
    int result = pthread_create(&threadInfo_, &threadAttr_, UpdateTicks, &g_ctx);
    assert(result == 0);//如果它的条件返回错误，则终止程序执行

    (void) result;//It works around some compiler warnings. Some compilers will warn if you don't use a function parameter.
    // In such a case, you might have deliberately not used that parameter, not be able to change the interface for some
    // reason, but still want to shut up the warning. That (void) casting construct is a no-op that makes the warning go away.
